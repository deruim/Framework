...
class Hayonik
{
    ...
    public function StartDownload()
    {
        ...
        curl_setopt($hayamax_curl, CURLOPT_URL, 'http://webmax.hayamax.com.br/crossdock/servlet/CrossDockingServlet.class.php?action=crossDockingPrice&customerId=' . $_SESSION['config.json']['distribuidores']['hayonik']['usuario'] . '&compress=' . $_SESSION['config.json']['distribuidores']['hayonik']['senha'] . '&canal=' . $_SESSION['config.json']['distribuidores']['hayonik']['canal']);
        $produtos_xml = curl_exec($hayamax_curl);
        curl_close($hayamax_curl);

        $hayamax_cache = fopen($_SESSION['config.json']['diretorios_cache']['1'] . 'produtos_hayonik.cache', 'w');
